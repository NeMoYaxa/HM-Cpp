#pragma once

// Находит произведение всех целых чисел, кратных трём, от a до b включительно
long long int productMultipleTree(int, int);

// Выводит стоимость 1.2, 1.4, …, 2 кг конфет
void priceCandies(double);

// Находит сумму степеней числа a
double powNumber(double, int);

// Находит приближенное значение sin(x)
double rowSin(double, int);

// Находит n-ое число Каталана
int numberCatalan(int);

//// Формирует новое число, нечетные разряды которого совпадают с соответствующими разрядами исходного числа, а четные равны нулю
//void numberTransform(int&);